...
void DetectNullPot()
{
  ...
  uint4 temp = get_no_p();
	...
	if ((temp == TRUE) && (statusViewNum & temp_2))
	{
		//检测到无锅且无锅
    statueViewCheckTime[0]=0;
		return;
...
{
	if(PWMRun() && !(statusViewNum & ((uint16)1 << 15)))
	{
		statueViewCheckTime[16]++;
		if (statueViewCheckTime[16] >= 8)
		{
			statueViewCheckTime[16]=0;
			statusViewNum |= ((uint16)1 << 15);//置1 不正常
		}
	}
	else
	{
